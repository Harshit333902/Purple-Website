...
let cartItems = [];
let destination = ''; // This will store the current folder path (uploads/ or images/)

// Function to add items to the cart
function addToCart(itemName, price, image, desti) {
    // Update destination based on the folder path passed
    destination = desti;

    // Ensure there's no double slash between destination and image
    const fullImagePath = destination.endsWith('/') ? destination + image : destination + '/' + image;

    // Add item to cart array with the full image path
    cartItems.push({ name: itemName, price: price, image: fullImagePath });
    ...
    cartItems.forEach(item => {
        ...
        cartItem.innerHTML = `
            <div class="cart-item-content">
                <!-- Dynamically add image based on the folder path -->
